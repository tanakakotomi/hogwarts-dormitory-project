...
- スリザリン選択時の分岐画面: 診断がスリザリンに決まる前に、ユーザーへ「スリザリンに行く」か「スリザリンは嫌だ！もう一回診断して！」を選べる確認画面を挟む
...
注: 質問の選択肢に追加変更があります。好きな魔法生物を選ぶ質問には `ドビー` と `ディメンター` を追加しています。
...
- `src/components/CharacterCard.vue`: キャラクター1件分の表示
 - `src/components/SlytherinChoice.vue`: スリザリンに診断される直前に表示する確認画面（「スリザリンに行く」/「スリザリンは嫌だ！もう一回診断して！」を選択）
